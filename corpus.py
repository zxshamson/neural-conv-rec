...
class Corpus:
    ...
    def __init__(self, filename, maxwordnum, pred_percentage):
        ...
        # self.time = {}         # Record each turn arriving time of each conv(if no time information, store turn nums)
        ...
        with open(filename, 'r') as f:
            ...
            for line in f:
                ...
                if msg_temp[msgs[1]] < conv_temp[i]:  # Be the training part
                    current_turn = [u, msg_temp[msgs[1]], msg_temp[msgs[2]], -1]
                    current_turn.extend(words)
                    try:
                        self.convs[i].append(current_turn)
                        # each turn of convs is a list of [user, turn_id, parent_turn_id, first_child_id, words]
                    except KeyError:
                    ...
                    try:
                        ...
                        self.pred_reply[i] = [u]
        # Find the first child, if no then still -1
        for key in self.convs.keys():
            first_child = {}
            for turn in self.convs[key]:
                if turn[2] not in first_child.keys():
                    first_child[turn[2]] = turn[1]
            for turn in self.convs[key]:
                try:
                    turn[3] = first_child[turn[1]]
                except KeyError:
                    sys.exc_clear()
